    ...
    ui(new Ui::PredictWindow)
{
    ...
    ui->setupUi(this);
    this->setWindowTitle("基于机器学习的智能股市预测");
    QPixmap pixmap=this->style()->standardPixmap(QStyle::SP_TitleBarNormalButton);
    this->setWindowIcon(pixmap);
    connect(networkManager, &QNetworkAccessManager::finished, this, &PredictWindow::onNetworkReply);
    connect(ui->returnButton, &QPushButton::clicked, this, &PredictWindow::onReturnButtonClicked);
...
void PredictWindow::paintEvent(QPaintEvent *event)
{
    ...
    if (stockDataList.size() >= 5) {
        ...
        painter.setPen(Qt::black);
        painter.drawText(predictedX - 140, predictedOpenY, QString("预测开盘: %1").arg(predictedOpen, 0, 'f', 2));
        painter.drawText(predictedX - 140, predictedCloseY, QString("预测收盘: %1").arg(predictedClose, 0, 'f', 2));
        painter.drawText(predictedX - 150, predictedCloseY + 20, QString("预测明日变化: %1%").arg(changePercent, 0, 'f', 2));
    }
...
void PredictWindow::onReturnButtonClicked()
{
    MainWindow *mainWindow = new MainWindow(); // Assuming MainWindow is the main menu class
    mainWindow->show();
    this->close();
}
