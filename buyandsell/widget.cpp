...
void Widget::on_confirmButton_clicked()
{
    ...
    QSqlQuery query(db);
    bool result =query.exec("insert into student(股票名称,股票代码,数量,当前价,买入或卖出,交易时间,差价) values(':name,':code',':num',':price',':buyorsell',':time',':delta')");
...
QString Widget::getcodeprice(QString stockcode)
{
    ...
    loop.exec();
    auto text=reply->readAll();
    QString qreg="\"cur_price\":\"([^\"]+)";//qt不支持汉字的unicode编码
    QRegularExpression re(qreg);
    auto pos=re.globalMatch(text);
    qDebug()<<pos.hasNext();
    while(pos.hasNext())
